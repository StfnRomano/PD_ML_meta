...
ggsave(filename = "/16S/R_analyses/Figures/AUC_PrevMLnorm.jpg", 
 	plot = g, 
 	device = "jpg", 
 	width = 8.5, height = 6)
...
# t-test
l_df.rar.w<-lapply(l_df.rar, function(x) t.test(x$not_rar, x$rar, paired = T, var.equal = F))
l_df.rar.w

# Cohen's D
l_cd<-l_df.rar.w
for(i in 1:length(l_df.rar)){
  tmp<-reshape2::melt(l_df.rar[[i]][,1:2])
   l_cd[[i]]<-rstatix::cohens_d(data = tmp,
                         formula = value~variable, 
                         paired = T, var.equal = F, ci =T)
}
l_cd
...
# extract CI
l.emm.ci<-lapply(l.emm, function(x) confint(x, level = 0.95)$contrasts %>%
                       as.data.frame)
df.emm.ci<-do.call(rbind, l.emm.ci)
df.emm.ci$ML.method<-c(rep("ENET", nrow(df.emm.ci)/6),
                  rep("LASSO", nrow(df.emm.ci)/6), 
                  rep("LASSO-LibLinear", nrow(df.emm.ci)/6),
                  rep("Random Forest", nrow(df.emm.ci)/6),
                  rep("Ridge regression", nrow(df.emm.ci)/6),
                  rep("Ridge-LibLinear", nrow(df.emm.ci)/6))
# combine the dfs
stopifnot(all(paste0(df.t$contrast, df.t$Validation.type) == paste0(df.emm.ci$contrast, df.emm.ci$Validation.type)))

# merge and save
df.emm.ci %>%
  left_join(df.t, by = c("contrast", "ML.method")) %>%
  select(-estimate.y, -SE.y, -df.y) %>%
  select(-ML.method, everything(), ML.method) %>%
  rename(estimate = estimate.x, SE = SE.x, df = df.x) %>%
  write.table(
            file = "/16S/R_analyses/RDS/LMM_Batch",
            quote = F,
            sep = "\t",
            row.names = F,
            col.names = T)
...
gen<-readRDS("/16S/R_analyses/RDS/meta.diff.gender.rds")
head(gen)
...
##################################################################
## test associations between pop characteristics and AUCs 
###################################################################
...
###############################################################################
# test effect of east west + Tukey
################################################################################
