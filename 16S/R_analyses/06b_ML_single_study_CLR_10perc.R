...
auc<-lasso_10x10@auc
lassoo.df<-data.frame(study = names(auc), prev = rep("10% in 10 std", 
                    length(lasso_10x10)), ML = rep("lasso", length(auc)),
                    AUC = unlist(lapply(auc, function(x) x$auc %>% as.double())),
                    norm = rep("clr", length(auc)))

print("Running lasso_ll")
lassoll_10x10<-siamcat.wf.lists(vector.names = study,
                       numCores = 10, df = all.tss, meta = meta, name.feat = "PD",  filt.method = "pass", 
                       cutoff = 0.30, log.n0 = min, nsplit = 1, n.fold = 10, 
                       trafo = "log.clr",  ml.method = "lasso_ll", 
                       raw.auc = F, plots = F,
                       max.show = 100, name.study.var = "Study_2", reps = 1)
auc<-lassoll_10x10@auc
lassoll.df<-data.frame(study = names(auc), prev = rep("10% in 10 std", 
                    length(lasso_10x10)), ML = rep("lasso_ll", length(auc)),
                    AUC = unlist(lapply(auc, function(x) x$auc %>% as.double())),
                    norm = rep("clr", length(auc)))

print("Running ridge")
ridge_10x10<-siamcat.wf.lists(vector.names = study,
                       numCores = 10, df = all.tss, meta = meta, name.feat = "PD",  filt.method = "pass", 
                       cutoff = 0.30, log.n0 = min, nsplit = 1, n.fold = 10, 
                       ...
                       max.show = 100, name.study.var = "Study_2", reps = 1)
auc<-ridge_10x10@auc
ridge.df<-data.frame(study = names(auc), prev = rep("10% in 10 std", 
...
ridge_ll_10x10<-siamcat.wf.lists(vector.names = study,
                       numCores = 10, df = all.tss, meta = meta, name.feat = "PD",  filt.method = "pass", 
                       cutoff = 0.30, log.n0 = min, nsplit = 1, n.fold = 10, 
                       trafo = "log.clr",  ml.method = "ridge_ll", 
                       raw.auc = F, plots = F,
                       max.show = 100, name.study.var = "Study_2", reps = 1)
auc<-ridge_ll_10x10@auc
ridgell.df<-data.frame(study = names(auc), prev = rep("10% in 10 std", 
...
print("Running enet")
enet_10x10<-siamcat.wf.lists(vector.names = study,
                       numCores = 10, df = all.tss, meta = meta, name.feat = "PD",  filt.method = "pass", 
                       cutoff = 0.30, log.n0 = min, nsplit = 1, n.fold = 10, 
...
auc<-enet_10x10@auc
enet.df<-data.frame(study = names(auc), prev = rep("10% in 10 std", 
...
rf_10x10<-siamcat.wf.lists(vector.names = study,
                       numCores = 10, df = all.tss, meta = meta, name.feat = "PD",  filt.method = "pass", 
                       cutoff = 0.30, log.n0 = min, nsplit = 1, n.fold = 10, 
...
auc<-rf_10x10@auc
rf.df<-data.frame(study = names(auc), prev = rep("10% in 10 std", 
...
saveRDS(df, paste0(args, "/RDS/Single_10per_clr.rds"))


## ---------------------------------------------------------------------------------------------
print("Saving wsp")
save.image(paste0(args, "/RData/16S_Sinlge_10perc_clr.RData"))
