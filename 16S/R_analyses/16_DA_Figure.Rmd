...
title <- ggdraw() + 
  draw_label(
    "Differentially abundant Genera between CTRL and PD",
    fontface = 'bold',
    x = 0.05,
    y = 0.7,
    hjust = 0
...
subtitle <- ggdraw() + 
  draw_label(
    "Generalised Odd Ratios were pooled using random effect meta-analysis. 16 studies, n=3165",size = 12,
    x = 0.05, y = 0.7,
    hjust = 0
  ) +
  theme(
...
```{r}
subset(df.sign,p.adjust < 0.05 & abs(TE.random) > 0.177 ) %>% dim()
subset(df.sign,p.adjust < 0.05 & abs(TE.random) > 0.177 ) -> df.sign
...
title <- ggdraw() + 
  draw_label(
    "Differentially abundant taxa between CTRL and PD",
...
subtitle <- ggdraw() + 
  draw_label(
    "Generalised Odd Ratios were pooled using random effect meta-analysis",size = 14,
    x = 0.05, y = 0.7,
    hjust = 0
  ) +
  theme(
...
gtgrob$grobs[[8]]$children[[2]]$grobs[[1]]$children[[1]]$gp$col <- factor(df.sign$dirDA, levels = c("black", "red"))
...
gtgrob$grobs[[9]]$children[[2]]$grobs[[1]]$children[[1]]$gp$col <- factor(df.sign.metaG$dirDA, levels = c("black", "red"))
