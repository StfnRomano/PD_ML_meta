...
## ---------------------------------------------------------------------------------------
all.tss<-readRDS(paste0(args, "/RDS/MMuphin_ad.rds"))
...
auc<-lasso_5x10@auc
lassoo.df<-data.frame(study = names(auc), prev = rep("5% in 10 std", 
...
lassoll.df<-data.frame(study = names(auc), prev = rep("5% in 10 std", 
...
ridge_5x10<-siamcat.wf.lists(vector.names = study,
                       ...
                       max.show = 100, name.study.var = "Study_2", reps = 1)
auc<-ridge_5x10@auc
ridge.df<-data.frame(study = names(auc), prev = rep("5% in 10 std", 
...
ridgell.df<-data.frame(study = names(auc), prev = rep("5% in 10 std", 
...
auc<-enet_5x10@auc
enet.df<-data.frame(study = names(auc), prev = rep("5% in 10 std", 
...
auc<-rf_5x10@auc
rf.df<-data.frame(study = names(auc), prev = rep("5% in 10 std", 
...
saveRDS(df, paste0(args, "/RDS/Single_5per_log_MMuphin.rds"))


## ---------------------------------------------------------------------------------------
print("Saving wsp")
save.image(paste0(args, "/RData/16S_Sinlge_5perc_log_MMuphin.RData"))
