...
auc<-lasso_20x10@auc
lassoo.df<-data.frame(study = names(auc), prev = rep("20% in 10 std", 
                    length(lasso_20x10)), ML = rep("lasso", length(auc)),
                    AUC = unlist(lapply(auc, function(x) x$auc %>% as.double())),
                    norm = rep("log", length(auc)))

print("Running lasso_ll")
lassoll_20x10<-siamcat.wf.lists(vector.names = study,
                       numCores = 10, df = all.tss, meta = meta, name.feat = "PD",  filt.method = "pass", 
                       cutoff = 0.30, log.n0 = min, nsplit = 1, n.fold = 10, 
                       trafo = "log.std",  ml.method = "lasso_ll", 
                       raw.auc = F, plots = F,
                       max.show = 100, name.study.var = "Study_2", reps = 1)
auc<-lassoll_20x10@auc
lassoll.df<-data.frame(study = names(auc), prev = rep("20% in 10 std", 
                    length(lasso_20x10)), ML = rep("lasso_ll", length(auc)),
                    AUC = unlist(lapply(auc, function(x) x$auc %>% as.double())),
                    norm = rep("log", length(auc)))

print("Running ridge")
ridge_20x10<-siamcat.wf.lists(vector.names = study,
                       numCores = 10, df = all.tss, meta = meta, name.feat = "PD",  filt.method = "pass", 
                       cutoff = 0.30, log.n0 = min, nsplit = 1, n.fold = 10, 
                       ...
                       max.show = 100, name.study.var = "Study_2", reps = 1)
auc<-ridge_20x10@auc
ridge.df<-data.frame(study = names(auc), prev = rep("20% in 10 std", 
...
ridge_ll_20x10<-siamcat.wf.lists(vector.names = study,
                       numCores = 10, df = all.tss, meta = meta, name.feat = "PD",  filt.method = "pass", 
                       cutoff = 0.30, log.n0 = min, nsplit = 1, n.fold = 10, 
                       trafo = "log.std",  ml.method = "ridge_ll", 
                       raw.auc = F, plots = F,
                       max.show = 100, name.study.var = "Study_2", reps = 1)
auc<-ridge_ll_20x10@auc
ridgell.df<-data.frame(study = names(auc), prev = rep("20% in 10 std", 
...
print("Running enet")
enet_20x10<-siamcat.wf.lists(vector.names = study,
                       numCores = 10, df = all.tss, meta = meta, name.feat = "PD",  filt.method = "pass", 
                       cutoff = 0.30, log.n0 = min, nsplit = 1, n.fold = 10, 
...
auc<-enet_20x10@auc
enet.df<-data.frame(study = names(auc), prev = rep("20% in 10 std", 
...
rf_20x10<-siamcat.wf.lists(vector.names = study,
                       numCores = 10, df = all.tss, meta = meta, name.feat = "PD",  filt.method = "pass", 
                       cutoff = 0.30, log.n0 = min, nsplit = 1, n.fold = 10, 
...
auc<-rf_20x10@auc
rf.df<-data.frame(study = names(auc), prev = rep("20% in 10 std", 
...
saveRDS(df, paste0(args, "/RDS/Single_20per_log.rds"))


## ---------------------------------------------------------------------------------------------
print("Saving wsp")
save.image(paste0(args, "/RData/16S_Sinlge_20perc_log.RData"))
