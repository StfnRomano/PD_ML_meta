...
#  LOG
##############
load(paste0(args, "/RData/16S_Sinlge_5perc_log_MMuphin.RData"))
...
l_csv_l5<-cross.study.validation(lasso_5x10,
                       microbiome = all.tss,
                       meta=meta,
                       names.studies = unique(meta$Study_2),
...
saveRDS(csv_log, paste0(args, "/RDS/CSV_5perc_Log_df_auc_MMuphin.rds"))


## --------------------------------------------------------------------------------------
print("Save wsp")
save.image(paste0(args, "/RData/Cross_validation_5perc_LOG_MMuphin.Rdata"))
