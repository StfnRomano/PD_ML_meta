...
list.of.packages <- c("dplyr", "ggplot2", "meta", "genodds")
...
# Filter
```{r}
spec<-readRDS("/metaG/R_analyses/Functions/AB_KOmod/RDS/spec_5x10.rds")
...
saveRDS(df.cross, "/metaG/R_analyses/Functions/AB_KOmod//RDS/df.cross.KOmod.rds")
...
# add description
mod<-read.delim("/metaG/R_analyses/Functions/KOmod/mod_filtered0.6_9May23_with.duplicates.txt", header = T)
mod<-mod[!duplicated(mod$description),8:9]



df.sign <- df.sign %>%
  left_join(mod, by = c("bins" = "num"))

saveRDS(df.sign, "/metaG/R_analyses/Functions/AB_KOmod//RDS/df.sign.KOmod.rds")
```
# Save workspace
```{r}
d<-"/metaG/R_analyses/Functions/AB_KOmod//RData/"
save.image(paste0(d, "KOmodDA_GenOdd.RData"))
```