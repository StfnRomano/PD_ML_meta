...
df.cross<-readRDS("/metaG//R_analyses/Functions/AA_KO/RDS/df.cross.GMM.rds")
...
df.pos.neg<-readRDS("/metaG//R_analyses/Functions/AA_KO/RDS//Ridge_df.pos.neg.GMM.rds")
...
title <- ggdraw() + 
  draw_label(
    "Differentially abundant GMMss between CTRL and PD",
    fontface = 'bold',
    x = 0.05,
    y = 0.7,
    hjust = 0
...
cowplot::save_plot(pgg, 
          filename = "/metaG/R_analyses//Functions/AA_KO/Figures/GMM.GenODDs_prev.svg", 
          base_width = 14, 
          base_height = 8)

write.table(df.sign, "/metaG/R_analyses/Functions/AA_KO/df.sign.GMM.txt",
            quote = F,
            col.names = T,
            row.names = F,
            sep = "\t")
write.table(df.pos.neg, "/metaG/R_analyses/Functions/AA_KO/df.pos.neg.GMM.txt",
            quote = F,
            col.names = T,
            row.names = F,
            sep = "\t")
...
supp.table<-data.frame(GMM_ID = df.sign$bins,
...
for(i in 1:nrow(supp.table)){
  ...
  supp.table$average.ridge.weight[i]<-df.pos.neg[df.pos.neg$taxa == supp.table$GMM_ID[i],"average"][1,1] %>% unlist() *-1
  supp.table$stdev.ridge.weight[i]<-df.pos.neg[df.pos.neg$taxa == supp.table$GMM_ID[i],"stdev"][1,1] %>% unlist()
}
...
write.table(x = supp.table, 
            file = "/metaG//R_analyses/Functions/AA_KO/RDS/DA.table.complete_GMM.txt",
            col.names = T, 
            row.names = F, 
            quote = F,
            sep = "\t")
