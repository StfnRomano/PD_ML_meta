...
# KO
KO<-readRDS("/metaG/R_analyses/Functions/AA_KO/RDS/Single_5per_log.w2500.rds")
KO$funct<-rep("KO", nrow(KO))
load("/metaG/R_analyses/Functions/AA_KO/RData/metaG_Sinlge_5perc_log_Wallen.w2500.RData")
df<-data.frame(
...
df.emm$Validation.type<-c(rep("CSV", 
                              nrow(df.emm)/3),
                          rep("LOSO", 
                              nrow(df.emm)/3), 
                          rep("within-study CV", 
...
# add 95% ci
l.emm.ci.est<-lapply(l.emm.ci, function(x) confint(x, level = 0.95)$contrasts %>%
                       as.data.frame)
df.emm.ci<-do.call(rbind, l.emm.ci.est)
df.emm.ci$Validation.type<-c(rep("CSV", 
                                 nrow(df.emm.ci)/3),
                             rep("LOSO", 
                                 nrow(df.emm.ci)/3), 
                             rep("within-study CV", 
...
# combine the dfs
stopifnot(all(paste0(df.emm$contrast, df.emm$Validation.type) == paste0(df.emm.ci$contrast, df.emm.ci$Validation.type)))

# merge
df.emm.ci %>%
  left_join(df.emm, by = c("contrast", "Validation.type")) %>%
  select(-estimate.y, -SE.y, -df.y) %>%
  select(-Validation.type, everything(), Validation.type) %>%
  rename(estimate = estimate.x, SE = SE.x, df = df.x) %>%
  write.table(
            file = "/metaG/R_analyses/Functions/KO/RDS/Table.emmeans.compare.AUCs.txt",
            quote = F,
            sep = "\t",
            row.names = F,
            col.names = T)
