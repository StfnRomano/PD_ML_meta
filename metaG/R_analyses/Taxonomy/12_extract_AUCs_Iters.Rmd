...
```{r}
path.in<-"/metaG/R_analyses/Taxonomy/RData/metaG_Sinlge_5perc_log.RData" 
path.out<-"/metaG/R_analyses/Taxonomy/RDS/" 
alg<-"ridge" 
...
for(i in 1:length(std)){
  ...
  fit<-mod[[s]]
  # extract all models
  fit.roc.all<-lapply(fit, function(x) x@eval_data$roc_all$auc %>%    
                        as.double())
...
Load work space for the single study models    
```{r}
path.in<-"/metaG/R_analyses/Taxonomy/RDS/All_metaG_loso_ridge.rds" 
path.out<-"/metaG/R_analyses/Taxonomy/RDS/" 
