...
auc<-lasso_20x10@auc
lassoo.df<-data.frame(study = names(auc), prev = rep("20% in 2 std", 
...
lassoll.df<-data.frame(study = names(auc), prev = rep("20% in 2 std", 
...
ridge_20x10<-siamcat.wf.lists(vector.names = study,
                       ...
                       max.show = 100, name.study.var = "Study", reps = 1)
auc<-ridge_20x10@auc
ridge.df<-data.frame(study = names(auc), prev = rep("20% in 2 std", 
...
ridgell.df<-data.frame(study = names(auc), prev = rep("20% in 2 std", 
...
auc<-enet_20x10@auc
enet.df<-data.frame(study = names(auc), prev = rep("20% in 2 std", 
...
auc<-rf_20x10@auc
rf.df<-data.frame(study = names(auc), prev = rep("20% in 2 std", 
...
## ------------------------------------------------------------------------------
print("Saving wsp")
save.image(paste0(args, "/RData/metaG_Sinlge_20perc_log.RData"))
