...
saveRDS(df_comb, "/metaG/R_analyses/Taxonomy/RDS/df.comb.cv.scv.rds")

# load loso
loso<-readRDS("//metaG//R_analyses/Taxonomy/RDS/All_metaG_loso_ridge.rds")
ls.out<-loso@auc
ls.out$training<-rep("All studies", nrow(ls.out))
ls.out$AUC<-ls.out$AUC* 100
ls.out$method<-rep("LOSO", nrow(ls.out))

saveRDS(ls.out, "/metaG/R_analyses/Taxonomy/RDS/ls.out.cv.scv.rds")
```
