...
auc<-lasso_30x10@auc
lassoo.df<-data.frame(study = names(auc), prev = rep("30% in 2 std", 
...
lassoll.df<-data.frame(study = names(auc), prev = rep("30% in 2 std", 
...
ridge_30x10<-siamcat.wf.lists(vector.names = study,
                       ...
                       max.show = 100, name.study.var = "Study", reps = 1)
auc<-ridge_30x10@auc
ridge.df<-data.frame(study = names(auc), prev = rep("30% in 2 std", 
...
ridgell.df<-data.frame(study = names(auc), prev = rep("30% in 2 std", 
...
auc<-enet_30x10@auc
enet.df<-data.frame(study = names(auc), prev = rep("30% in 2 std", 
...
auc<-rf_30x10@auc
rf.df<-data.frame(study = names(auc), prev = rep("30% in 2 std", 
...
saveRDS(df, paste0(args, "/RDS/Single_30per_log.rds"))


## ------------------------------------------------------------------------------
print("Saving wsp")
save.image(paste0(args, "/RData/metaG_Sinlge_30perc_log.RData"))
